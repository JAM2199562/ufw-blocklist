...
check_requirements() {
    ...
    if [ -f /etc/os-release ]; then
        . /etc/os-release
        if [[ "$ID" != "ubuntu" ]]; then
            print_error "This installer requires Ubuntu OS"
            exit 1
        fi
...
EOF
        ...
        print_status "$YELLOW" "  ⚠ Configuration file already exists"
...
install_scripts() {
    ...
    print_status "$GREEN" "    ✓ ufw-blocklist-config → /usr/local/bin/ufw-blocklist-config"

    # Create module directory
    if [ ! -d "$MODULE_DIR" ]; then
        mkdir -p "$MODULE_DIR"
        print_status "$GREEN" "    ✓ Created module directory: $MODULE_DIR"
    fi
...
download_initial_data() {
    ...
    if [ "$ENABLE_THREAT_UPDATE" = "yes" ]; then
        ...
        if curl -sS -f --compressed "$THREAT_URL" -o "/etc/ipsum.3.txt"; then
            ...
            print_status "$GREEN" "    ✓ Downloaded $threat_count threat IPs"
        else
        ...
        print_status "$YELLOW" "  ⚠ Threat intelligence download disabled"
    fi
    ...
    if [ "$ENABLE_GEO_UPDATE" = "yes" ]; then
        ...
        if curl -sS -f "$GEO_URL" -o "/etc/cn.zone"; then
            ...
            print_status "$GREEN" "    ✓ Downloaded $geo_count geographic IP ranges"
        else
        ...
        print_status "$YELLOW" "  ⚠ Geographic IP download disabled"
    fi
...
║                                                                      ║
║  UFW Blocklist Enhanced Edition v$VERSION has been installed.          ║
║                                                                      ║
║  Configuration Summary:                                               ║"
    print_status "$BLUE" "║  Threat Intelligence: $([ "$ENABLE_THREAT_UPDATE" = "yes" ] && echo "Enabled" || echo "Disabled")"
    print_status "$BLUE" "║  Geographic Blocking: $([ "$ENABLE_GEO_UPDATE" = "yes" ] && echo "Enabled" || echo "Disabled")"
    print_status "$BLUE" "║  Update Frequencies: Threat=$THREAT_UPDATE_FREQ, Geo=$GEO_UPDATE_FREQ"
    print_status "$BLUE" "║  Default Strategy: $DEFAULT_STRATEGY"
    print_status "$BLUE" "║                                                                      ║
║  Next Steps:                                                         ║
║  1. Test configuration: sudo ufw-blocklist-config --show         ║
║  2. Enable firewall: sudo ufw enable                              ║
║  3. Check status: sudo ufw status                                  ║
║  4. Reconfigure anytime: sudo ufw-blocklist-config --interactive ║
╚════════════════════════════════════════════════════════════════╝"

    if [ "$DEFAULT_STRATEGY" = "conservative" ]; then
        print_status "$YELLOW" "
🔒 Conservative Strategy: Only threat intelligence is enabled."
        print_status "$YELLOW" "   Geographic blocking can be enabled with:"
        print_status "$YELLOW" "   sudo ufw-blocklist-config --interactive"
    fi

    print_status "$GREEN" "
🎉 Installation successful! Your Ubuntu firewall is now enhanced with advanced IP blocking."
}
...
case "${1:-install}" in
    ...
    install|"")
        ...
        print_status "$GREEN" "
╔══════════════════════════════════════════════════════════════╗
║              UFW Blocklist Enhanced Installer v$VERSION              ║
